...
async def main_functions(zed_signal_attrs, lock2):    

    async with websockets.connect(f"ws://10.20.24.10:5555/ws", ping_interval=5, ping_timeout=5) as ws:
        while True:
            tasks = [ 
            asyncio.ensure_future(recive_data(zed_signal_attrs, lock2)),                                      # running concurrently:    
            # asyncio.ensure_future(decision()),               # decides which state per 10 readings
            asyncio.ensure_future(reset(ws)),           # decides which behaviours
